...
final class AdminController extends Controller
{
	...
	public function saveCatalog(): void { AdminMiddleware::handle();if(!CSRF::verify($_POST['_csrf']??null))$this->redirect('/admin');$table=$_POST['table']==='skills'?'skills':'categories';$name=trim((string)$_POST['name']);$slug=trim((string)$_POST['slug']);if($name!==''&&$slug!==''){ $db=Database::connection();if($table==='skills'){$s=$db->prepare('INSERT INTO skills(name,slug,category_id,description,icon) VALUES(:name,:slug,:category,:description,:icon) ON DUPLICATE KEY UPDATE name=VALUES(name),category_id=VALUES(category_id),description=VALUES(description),icon=VALUES(icon)');$s->execute(['name'=>$name,'slug'=>$slug,'category'=>(int)($_POST['category_id']??0)?:null,'description'=>trim((string)$_POST['description']??''),'icon'=>trim((string)$_POST['icon']??'')?:null]);}else{$s=$db->prepare('INSERT INTO categories(name,slug,description,icon,parent_id) VALUES(:name,:slug,:description,:icon,:parent) ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),icon=VALUES(icon),parent_id=VALUES(parent_id)');$s->execute(['name'=>$name,'slug'=>$slug,'description'=>trim((string)$_POST['description']??''),'icon'=>trim((string)$_POST['icon']??'')?:null,'parent'=>(int)($_POST['parent_id']??0)?:null]);}}$this->redirect('/admin/'.$table); }
	public function deleteCatalog(): void { AdminMiddleware::handle();if(CSRF::verify($_POST['_csrf']??null)){$table=$_POST['table']==='skills'?'skills':'categories';Database::connection()->prepare("DELETE FROM {$table} WHERE id=:id")->execute(['id'=>(int)$_POST['id']]);}$this->redirect('/admin/'.$table); }
	public function reportStatus(): void { AdminMiddleware::handle();if(CSRF::verify($_POST['_csrf']??null)){Database::connection()->prepare('UPDATE reports SET status=:status,resolution=:resolution WHERE id=:id')->execute(['status'=>in_array($_POST['status'],['open','reviewing','resolved','dismissed'],true)?$_POST['status']:'reviewing','resolution'=>trim((string)$_POST['resolution']),'id'=>(int)$_POST['id']]);}$this->redirect('/admin/reports'); }
	...
	private function resourcePage(string $table, string $title, string $description): void
	{
		...
		$search=trim((string)($_GET['q']??''));$page=max(1,(int)($_GET['page']??1));$limit=25;$where='';$params=[];$searchColumns=array_values(array_intersect(['title','name','slug','status','email','reason','target_type'],$available));if($search!==''&&$searchColumns){$where=' WHERE '.implode(' OR ',array_map(fn($c)=>"`$c` LIKE :q",$searchColumns));$params['q']='%'.$search.'%';}$count=$db->prepare("SELECT COUNT(*) FROM `{$table}`{$where}");$count->execute($params);$total=(int)$count->fetchColumn();$pages=max(1,(int)ceil($total/$limit));$offset=($page-1)*$limit;$sql='SELECT '.implode(',',array_map(fn($c)=>"`$c`",$columns))." FROM `{$table}`{$where} ORDER BY `created_at` DESC LIMIT {$limit} OFFSET {$offset}";$stmt=$db->prepare($sql);$stmt->execute($params);$rows=$stmt->fetchAll();$this->view('admin/resource',compact('title','description','table','columns','rows','search','page','pages','total'),'admin');
	}
