...
final class Portfolio extends Model
{
	...
	public function updateOwned(int $id,int $userId,array $data): void { $s=$this->db->prepare('UPDATE portfolios SET title=:title,description=:description,project_url=:project_url,github_url=:github_url,technologies=:technologies,project_result=:project_result WHERE id=:id AND user_id=:uid');$s->execute(['id'=>$id,'uid'=>$userId,'title'=>$data['title'],'description'=>$data['description'],'project_url'=>$data['project_url']?:null,'github_url'=>$data['github_url']?:null,'technologies'=>$data['technologies']?:null,'project_result'=>$data['project_result']?:null]); }
	public function deleteOwned(int $id,int $userId): void { $s=$this->db->prepare('DELETE FROM portfolios WHERE id=:id AND user_id=:uid');$s->execute(['id'=>$id,'uid'=>$userId]); }
