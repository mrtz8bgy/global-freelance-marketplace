...
final class Contract extends Model
{
    public function forUser(int $userId, string $role): array
    {
        ...
        $s = $this->db->prepare("SELECT c.*, j.title, p.display_name AS other_party FROM contracts c INNER JOIN jobs j ON j.id=c.job_id LEFT JOIN profiles p ON p.user_id=CASE WHEN c.employer_id=:party_uid THEN c.freelancer_id ELSE c.employer_id END WHERE {$field}=:filter_uid ORDER BY c.created_at DESC");
        $s->execute(['party_uid'=>$userId, 'filter_uid'=>$userId]); return $s->fetchAll();
    }
